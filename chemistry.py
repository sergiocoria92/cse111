...
NAME_INDEX = 0
ATOMIC_MASS_INDEX = 1
ATOMIC_NUMBER_INDEX = 2  # New index for atomic number
...
def make_periodic_table():
    ...
    Now includes atomic numbers for each element.
    
    Returns:
        dict: A dictionary where keys are element symbols and values are [name, atomic_mass, atomic_number]
    """
    periodic_table_dict = {
        "Ac": ["Actinium", 227, 89],
...
def main():
    """Main function that calculates and prints molar mass, number of moles, 
    compound name, and total protons."""
    ...
    # Get the periodic table and parse the formula
    ...
    print(f"Total protons in molecule: {total_protons}")
